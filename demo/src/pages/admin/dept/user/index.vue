<template>
  <Table
    :hideCardBorder="props.hideCardBorder"
    :fields="table.fields"
  ...
  >
    ...
    <template #deptIds="{ record }">
      <div :style="{ width: `280px` }" v-if="record.dept_list.length > 0">
        <a-overflow-list>
          <a-tag v-for="dept of record.dept_list" :key="dept">{{ dept }}</a-tag>
        </a-overflow-list>
      </div>
      <div v-else>-</div>
    </template>
...
onMounted(() => {
  if (typeof props.dept_id !== 'undefined') {
    // 只在组织管理内处理组织或岗位归属
    table.ignoreFields.push('deptIds');
    table.ignoreFields.push('range');
...
const getDatas = async () => {
  const {
    data: { fields, data, total, dept_list },
  } = await userQuery({
    ...table.form,
    ...table.pagination,
  });
  table.dept_list = dept_list;
  table.datas = data;
  table.fields = fields;
...
const switchRange = (record) => {
  userUpdate({ id: record.id, range: record.range }).then((res) => {
